...
/**
 * @param array $collections - usually $data['Group'] from $Model->find()
 */
	...
	if ($isXhr) {
		...
		if ($isInner) {
			// from paginator
			echo $this->element('/collections/paging-inner', compact('isPreview', 'isWide', 'total', 'controllerAttrs'));
		} else {
...
<?php 
		...
		echo $this->element('/collections/section-header');
