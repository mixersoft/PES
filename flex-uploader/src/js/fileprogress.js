...
(function() {
    ...
    SNAPPI.onYready.FileProgress = function(Y){
		if (_Y === null) _Y = Y;
	 ...
	 * @params page int or _Y.Node - for access to the page node
	 */
	var FileProgress = function (cfg, page){
		this._cfg = _Y.merge(Factory.PhotoAirUpload.defaultCfg, cfg);
	    ...
	    this.height = 0;		// deprecate, used by appear()/disappear()
	        // use YUI3 libs
	//LOG("******** FileProgress constructor  _Y.version = " + _Y.version + ", cfg.label=" + cfg.label + ", cfg.uuid=" + cfg.uuid);
	        // init upload-queue-container
	    var node = _Y.one('#progress-' + this._cfg.uuid);
	    if (!node) {		// create node
	        // use ThumbnailFactory for markup init
	        node = _Y.Node.create(Factory.PhotoAirUpload.markup);
	        ...
	        if (page instanceof _Y.Node) {
	            page.append(this.node);
	        } else {
	        	try {
	                _Y.one('.gallery .container').append(this.node);
	        	} catch (e) {
