...
print_r("<br>new ShellDispatcher args=");
print_r($args); 		
		set_time_limit(0);
		$this->__initConstants();
		$this->parseParams($args);
		if (defined('THRIFT_LIB_BASEPATH')) {
			$this->_initEnvironment_Thrift();	
		} else $this->_initEnvironment();
		$this->__buildPaths();
		// $this->_stop($this->dispatch() === false ? 1 : 0);
		
		/*
		 * boostrap cake console shell,
		 * but DO NOT EXIT SCRIPT
		 * use $this-Shell to access Shell Models in Thrift server  
		 */ 
		$this->Shell = $this->dispatch();
print("\r\n<br>ShellDispatcher::constructor() complete");
	}
 ...
 */
	function __initConstants() {
		...
		if (!defined('CAKE_CORE_INCLUDE_PATH')) {
			...
			// snaphappi CAKE_CORE_INCLUDE_PATH
			$core = dirname(dirname(dirname(__FILE__))).'cake-core';
		    if (!file_exists($core)) {
		        $core = dirname($core).DS.'cake-core';
		        if (!file_exists($core)) {
		            throw new Exception("set cake-core for cake bootstrap");
		        }
		    }			
			define('CAKE_CORE_INCLUDE_PATH', $core);
...
/**
 * Defines current working environment. (from console)
 *
 * @access protected
 */
	function _initEnvironment() {
		...
		$check = $this->shiftArgs();
		print("\r\n<br>_initEnvironment shift, arg=".$check);	
	}
	
/**
 * Defines current working environment for thrift Server script
 *
 * @access protected
 */
	function _initEnvironment_Thrift() {
		if (!$this->__bootstrap()) {
			$this->_stop();
			throw new Exception("Error: Cake bootstrap failed.");
		}
		$check = $this->shiftArgs();
		print("\r\n<br>_initEnvironment_Thrift shift , arg=".$check);
	}	
...
print_r("\r\n<br>Vendors paths=".print_r($paths, true).'<br />');
...
print("\r\n<br>dispatch()  arg={$arg}<br>");
		...
		/*
		 * Just return $Shell for later use, i.e. $dispatcher->Shell->[{$Model}]
		 */
		try {
			$Shell->startup();
			return $Shell;
		} catch(Exception $e) {
			$title = sprintf(__('Error: Prolem initializing Shell %1$s.', true), $this->shellName);
			$this->stderr($title . "\n" . $message . "\n");
		}
 ...
 */
	function parseParams($params) {
		...
		/*
		 * HACK:
		 *  for loading bootstrap script from /BAKED/app/vendors/thrift
		 *  instead of cake-core/cake/console
		 */
		$defaults['root'] = dirname($defaults['root']); 
		/*
		 * 
		 */
...
if (!defined('DISABLE_AUTO_DISPATCH')) define('DISABLE_AUTO_DISPATCH', 1);
if (!defined('DISABLE_AUTO_DISPATCH')) {
	$dispatcher = new ShellDispatcher($argv);
}


print("\r\n<br> boostrap complete;");

/*
 * After require_once()
 * Shell, Models should be available for use in Thrift interface
 *
 */
 	// $dispatcher = new ShellDispatcher($argv);
// print("\r\n<br> new dispatcher<br>");	
	// $options = array(
		// 'fields'=>array("User.id, User.username"),
	// );
	// $data = $dispatcher->Shell->User->find('first', $options);
	// print_r($data);
	// try{	
		// $options = array(
		...
		// print_r(in_array('Permissionable', $Group->Behaviors->attached()));
		// print_r(isset($Group->Behaviors->Permissionable));
// 		
		// $Group->Behaviors->detach('Permissionable');
		// print_r($Group->Behaviors->attached());
		// $data = $Group->find('first', $options);
		// print_r($data);
	// }catch(Exception $e) {}
